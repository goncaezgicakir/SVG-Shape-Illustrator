...
//---------------------------COMPOSED SHAPES CLASS GETTERS------------------------------------------------------------

char ComposedShape::getShapeC() const
{   return shapeC;
}

inline double ComposedShape::getHeightC() const
{ return heightC;
}

inline double ComposedShape::getWidthC() const
{ return widthC;
}

inline double ComposedShape::getRadiusC() const
{   return radiusC;
}

inline double ComposedShape::getLengthC() const
{ return lengthC;
}

inline char ComposedShape::getShapeS() const
{
    return shapeS;
}

inline double ComposedShape::getHeightS() const
{ return heightS;
}

inline double ComposedShape::getWidthS() const
{ return widthS;
}

inline double ComposedShape::getRadiusS() const
{   return radiusS;
}

inline double ComposedShape::getLengthS() const
{ return lengthS;
}

vector <ComposedShape::ShapeElem> ComposedShape::getSmallShape() const
{
    return s;
}
...
inline Rectangle ComposedShape::getRectangle() const
{
    return R;
}
inline Triangle ComposedShape::getTriangle() const
{
    return T;
}
inline Circle ComposedShape::getCircle() const
{
    return C;
}
...
{
    if(compObject.getShapeC() == 'R'){
        compObject.getRectangle().draw(fp);
    }

    else if(compObject.getShapeC() == 'C')
    {
        compObject.getTriangle().draw(fp);
    }
    else if(compObject.getShapeC() == 'T')
    {
        compObject.getCircle().draw(fp);
    }
...
const ComposedShape ComposedShape::operator+=(const ComposedShape::ShapeElem& addObject)
{
    s.push_back(addObject);
    return *this;
...
void ComposedShape::draw(ofstream &fp)
{
...
void ComposedShape::optimalFit ()
{
   ...
   char smallShape = getShapeS();
    ...
    //assigning enum types 
    if (containShape == 'R')
        containShape = rectangle;

    if (containShape == 'T')
        containShape = triangle;

    if (containShape == 'C')
        containShape = circle;

    if (smallShape == 'R')
        smallShape = rectangle;
    ...
    //callin gfunctions to draw composed shape
    if (containShape == rectangle && smallShape == rectangle)
        RinR();

    if (containShape == rectangle && smallShape == circle)
        CinR();

    if (containShape == rectangle && smallShape == triangle)
        TinR();

    if (containShape == circle && smallShape == circle)
        CinC();

    if (containShape == circle && smallShape == rectangle)
        RinC();

    if (containShape == circle && smallShape == triangle)
        TinC();

    if (containShape == triangle && smallShape == triangle)
        TinT();

    if (containShape == triangle && smallShape == circle)
        CinT();

    if (containShape == triangle && smallShape == rectangle)
        RinT();
...
void ComposedShape::modifyR(int counter, double coorX, double coorY)
{
    ...
    //s.resize(s.size()+1);
    s[counter] = ComposedShape::ShapeElem(temp);
    //s.push_back(ComposedShape::ShapeElem(temp));
}
...
//----------------------NESTED CLASS "ShapeElem" CONSTRUCTORS AND BIG THREE-----------------------------------------------

ostream& operator <<(ostream& fp, const ComposedShape::ShapeElem& object)
{

   if(object.getShapeType() == rectangle)
        fp << *((Rectangle*)(object.getShapePtr()));

    if(object.getShapeType() == circle)
    {
        fp << *((Circle*)(object.getShapePtr()));
    }
    if(object.getShapeType() == triangle)
        fp << *((Triangle*)(object.getShapePtr()));
...
ComposedShape::ShapeElem::ShapeElem(const Rectangle& objectRec)
{
    Rectangle *temp = new Rectangle(objectRec);
    shapePtr = static_cast<void*>(temp);
    shapeType = rectangle;

}

ComposedShape::ShapeElem::ShapeElem(const Circle& objectCir)
{
    Circle *temp = new Circle(objectCir);
    shapePtr = static_cast<void*>(temp);
    shapeType = circle;
}

ComposedShape::ShapeElem::ShapeElem(const Triangle& objectTri)
{
    Triangle *temp = new Triangle(objectTri);
    shapePtr = static_cast<void*>(temp);
    shapeType = triangle;
}
...
//copy constructor
ComposedShape::ShapeElem::ShapeElem(const ShapeElem& object)
{

    if(shapeType == rectangle)
    {
        delete static_cast<Rectangle*>(shapePtr);
        shapePtr = new Rectangle(*((Rectangle*)(object.getShapePtr())));
    }
    else if(shapeType == circle)
    {
        delete static_cast<Circle*>(shapePtr);
        shapePtr = new Circle(*((Circle*)(object.getShapePtr())));
    }
    else if(shapeType == triangle)
    {
        delete static_cast<Triangle*>(shapePtr);
        shapePtr = new Triangle(*((Triangle*)(object.getShapePtr())));
    }

}

//overloaded assignment operator
ComposedShape::ShapeElem& ComposedShape::ShapeElem::operator =(const ComposedShape::ShapeElem& assignedObject)
{

   if(this != &assignedObject)
   {

        shapeType = assignedObject.getShapeType();


        if(shapeType == rectangle)
        {
            delete static_cast<Rectangle*>(shapePtr);
            shapePtr = new Rectangle(*((Rectangle*)(assignedObject.getShapePtr())));
        }
        else if(shapeType == circle)
        {
            delete static_cast<Circle*>(shapePtr);
            shapePtr = new Circle(*((Circle*)(assignedObject.getShapePtr())));
        }
        else if(shapeType == triangle)
        {
            delete static_cast<Triangle*>(shapePtr);
            shapePtr = new Triangle(*((Triangle*)(assignedObject.getShapePtr())));
        }
...
//------------------NESTED CLASS "ShapeElem" GETTERS, SETTERS AND FUNCTIONS---------------------------------------------------------------------

inline void* ComposedShape::ShapeElem::getShapePtr() const
{
    return shapePtr;
}
...
inline Shapes ComposedShape::ShapeElem::getShapeType() const
{
    return shapeType;
}


void ComposedShape::ShapeElem::setShapeType(Shapes typeOfShape)
{
