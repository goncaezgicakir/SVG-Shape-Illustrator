...
double ComposedShape::getHeightC() const
{ return heightC;
}

double ComposedShape::getWidthC() const
{ return widthC;
}

double ComposedShape::getRadiusC() const
{   return radiusC;
}

double ComposedShape::getLengthC() const
{ return lengthC;
}

char ComposedShape::getShapeS() const
{
    return shapeS;
}

double ComposedShape::getHeightS() const
{ return heightS;
}

double ComposedShape::getWidthS() const
{ return widthS;
}

double ComposedShape::getRadiusS() const
{   return radiusS;
}

double ComposedShape::getLengthS() const
{ return lengthS;
}

vector < Rectangle > ComposedShape::getSmallRectangle() const
{   return r;
}

vector < Circle > ComposedShape::getSmallCircle() const
{   return c;
}
vector < Triangle > ComposedShape::getSmallTriangle() const
{   return t;
}
...
{
    r.resize(r.size()+1);
    r[counter].setCoorX(coorX);
    r[counter].setCoorY(coorY);
    r[counter].setHeight(heightS);
    r[counter].setWidth(widthS);
...
void ComposedShape::modifyC(int counter, double coorX, double coorY)
{
    ...
    c.resize(c.size()+1);
    c[counter].setCoorX(coorX) ;
    c[counter].setCoorY(coorY) ;
    c[counter].setRadius(radiusS);
...
{
    t.resize(t.size()+1);
    vector<double> temp{coorX1, coorY1, coorX2, coorY2, coorX3, coorY3};
    t[counter].setCoordinate(temp);
    t[counter].setLength(lengthS);
...
void ComposedShape::RinR()
{
    ...
    for(i=0 ; i<(int)(cwidth/swidth) ; ++i)
    {
        coorX = swidth*i;

        for(j=0 ; j<(int)(cheight/sheight) ; ++j)
        {
            coorY = sheight*j;
    ...
    if(remainHeight>0) {
        ...
        for(i=0 ; i<(int)(cwidth/sheight) ; ++i)
        {
            coorX = sheight*i;

            for(j=0 ; j<(int)(remainHeight/swidth) ; ++j)
            {
    ...
    if(remainWidth>0){
        ...
        for(i=0 ; i<(int)(remainWidth/sheight) ; ++i)
        {
            coorX = (cwidth - remainWidth) + (sheight*i);

            for(j=0 ; j<(int)(cheight/swidth) ; ++j)
            {
                coorY = swidth*j;
...
void ComposedShape::CinR()
{
    ...
    for(i=0; i< (int)(cheight/(sradius*1.732)); ++i)
    {

        for(j=0; j<(int)(cwidth/(sradius*2)); ++j)
        {
...
void ComposedShape::CinC()
{
    ...
    for(i=0 ; i<(int)((2*cradius)/(sradius*1.732)) ; ++i)
    {

        for(j=0 ; j<(int)((2*cradius)/(2*sradius)) ; ++j)
        {
...
void ComposedShape::RinC()
{
    ...
    for(i=0 ; i<(int)((2*cradius)/swidth); ++i)
    {
        s_coorX = swidth * i;

        for (j = 0; j < (int)(2 * cradius) / sheight; ++j)
        {
            s_coorY = sheight * j;
...
void ComposedShape::RinT()
{
    ...
    for(i=0 ; i<(int)(((clength*0.866)/sheight)) ; ++i)
    {
        coorY = sheight *i;
...
void ComposedShape::CinT()
{
    ...
    for(i=(int)((clength*0.866) /(sradius * 1.732)) ; i>0 ; --i)
    {

        for (j = (int)((clength - (2 * (sradius * 1.732 - sradius))) / (2 * sradius)); j>0; --j)
        {
...
void ComposedShape::TinR()
{
    ...
    for(i=0 ; i<(int)(cheight/(slength*1.732))  ; ++i)
    {

        for(j=0 ; j<(int)(cwidth/slength) ; ++j)
        {
...
void ComposedShape::TinC()
{
    ...
    for(i=0 ; i<(int)((2*cradius)/(slength*1.732)) ; ++i)
    {

        for(j=0 ; j<(int)((2*cradius)/slength); ++j)
        {
            s_x1 = j * slength;
            s_y1 = i * slength * 1.732;
            s_x2 = slength + j * slength;
